...
create_sale_history_plot <- function(account_num){
    ...
    plot <- ggplot(data = xfer_history_tbl, aes(x = xfer_date, y = numeric_consideration)) +
        ...
        xlab("\nTransfer Date") +
        ylab("Consideration\n") +
        labs(title = paste("Sales History for",title_txt)) +
...
create_sales_by_year_plot <- function(){
    ...
    sales_grouped_by_year_plot <-
        ggplot(data = sales_by_year_tbl, aes(x = year, y = count)) +
        geom_point() +
        geom_line() +
        xlab("Year of Sale") +
        ylab("Count of Sales") +
        scale_x_continuous(breaks = seq(min(sales_by_year_tbl$year), max(sales_by_year_tbl$year), by = 5)) + # Set breaks every 5 years
        labs(
            title = "Count of Sales by Year",
            subtitle = "Number of sales of houses in Emerald Gardens by year<br/>(excludes transfers where beneficial ownership does not change)") +
        theme_economist_wheat_bkgrnd() +
    ...
    # Add subtitle (without clearing annotations)

    # Append subtitle
    subtitle_text <- "<br><span style='font-size: 12px; line-height: 0.5;'><b>Number of sales of houses by year</b></span>"
    ggp$x$layout$title$text <- paste0(ggp$x$layout$title$text, subtitle_text)
    ggp$x$layout$title$x <- 0.5
    ggp$x$layout$title$xref <- "paper"
...
generate_expected_just_range <- function(model){
    ...
    # Compute 2-sigma lines
    augmented_data <- augmented_data %>%
        mutate(
            upper_2sigma = .fitted + 2 * sigma_val,
            lower_2sigma = .fitted - 2 * sigma_val,
            fitted_exp = exp(.fitted),      # Transform fitted values HERE
            log_just_exp = exp(log_just),   # Transform log_just values HERE
            upper_2sigma_exp = exp(upper_2sigma), #Transform upper 2-sigma values HERE
            lower_2sigma_exp = exp(lower_2sigma) #Transform lower 2-sigma values HERE
...
create_model_dot_plot <- function(model){
    ...
    augmented_data <- generate_expected_just_range(model)

    # Plot actual vs. predicted values
    plot <- ggplot(augmented_data, aes(x = exp(.fitted), y = exp(log_just))) +
        geom_point(color = "royalblue", alpha = 0.9) +
        geom_smooth(method = "lm", color = "tomato2", se = TRUE) +
        geom_line(aes(y = upper_2sigma_exp), color = "steelblue", linetype = "dashed", linewidth = 0.8) +
        geom_line(aes(y = lower_2sigma_exp), color = "steelblue", linetype = "dashed", linewidth = 0.8) +
        scale_x_continuous(labels = scales::label_dollar()) +
        scale_y_continuous(labels = scales::label_dollar()) +
        labs(
            title = "Actual vs. Predicted House Valuations with ±2σ Bounds",
            subtitle = "Points above and below the dashed line are outliers.",
            x = "Predicted Value (Fitted)\n",
            y = "'Just' Value"
        ) +
        theme_economist_wheat_bkgrnd()
    ...
    # Tooltip Adjustments (as before)
    plot$x$data[[1]]$text <- paste0(
        str_to_title(augmented_data$addr), "<br />",
        str_replace(plot$x$data[[1]]$text, "exp\\(.fitted\\)", "Predicted value") %>%
            str_replace(., "exp\\(log_just\\)", "Just Value") %>%
...
create_coefficients_table_html <- function(summary_output) {
    ...
    # Add coefficient names as a new column
    coef_df$Coefficient <- rownames(coef_df)

    # Reorder columns to put coefficient names first
    coef_df <- coef_df[, c("Coefficient", colnames(coef_df)[-ncol(coef_df)])]
...
# Writer function to save data for a plot into a tibble.
# Data retrieved in the app.R server block.
#
# Called once per model in app.R (e.g., q_q_plot_of_residuals, residuals plot)
save_plot_data <- function(model, filename) {
    # 1. Extract raw values
    res_vals  <- if ("rlm" %in% class(model)) residuals(model)/model$s else rstandard(model)
    fit_vals  <- as.numeric(fitted(model))

    # 2. Build a combined "Data Package"
    # We use a list so we can store the scatter data AND the QQ math together
    plot_data_bundle <- list(
        scatter = tibble::tibble(
            fitted = fit_vals,
            residuals = as.numeric(resid(model))
        ),
        qq = tibble::tibble(
            theoretical = qnorm(ppoints(length(res_vals))),
            sample = sort(as.numeric(res_vals)),
            slope = sd(res_vals)
        ),
        formula_label = paste(as.character(formula(model))[2], "~", as.character(formula(model))[3])
    )

    saveRDS(plot_data_bundle, paste0("03_plot_files/", filename, ".rds"))
}
...
pseudo_r_squared <- function(model) {
    # Check if the model is a glm object (logistic, poisson, etc.)
    if (inherits(model, "glm")) {
        # Calculate null deviance and residual deviance
        ...
        # For linear models, use the regular R-squared
...
# Pseudo R-Squared
# pseudo_r_squared(create_glm_4a_log())
